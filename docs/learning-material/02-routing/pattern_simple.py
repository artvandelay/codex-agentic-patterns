...
def handle_generate(query: str) -> str:
    ...
    response = client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=[{"role": "user", "content": f"Generate the following: {query}"}],
    )
    return response.choices[0].message.content
...
def handle_analyze(query: str) -> str:
    ...
    response = client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=[{"role": "user", "content": f"Analyze and explain: {query}"}],
    )
    return response.choices[0].message.content
...
def keyword_based_routing_example():
    ...
    def classify_by_keywords(query: str) -> str:
        ...
        if any(word in query_lower for word in ["search", "find", "what is", "who is"]):
            ...
            word in query_lower for word in ["calculate", "compute", "sum", "multiply"]
        ):
            return "calculate"
        elif any(
            word in query_lower for word in ["generate", "create", "write", "make"]
        ):
            return "generate"
